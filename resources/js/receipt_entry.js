    ...
    $(document).ready(function(){
        ...
        $(document).on('change', '#test_by', function(){
            $("#test_by").siblings().children().children().css('background-color', 'transparent');
        });
        $(document).on('change', '#blood_grp', function(){
            $("#blood_grp").siblings().children().children().css('background-color', 'transparent');
        });
        $(document).on('change', '#patient_id', function(){
            $("#patient_id").siblings().children().children().css('background-color', 'transparent');
        });
        ...
        $('#radioDiv input[type=radio]').change(function(){
            if($(this).val() == '0')
            {
                ...
                $('#patient_nm').prop('disabled', false);
                $('#patient_age').prop('disabled', false);
                $('#gender').prop('disabled', false);
                $('#blood_grp').prop('disabled', false);
                $('#cont_num').prop('disabled', false);
    ...
    $(document).on('click', '#get_details', function(){
        ...
        patient_id = $('#patient_id option:selected').val();
        
        if(patient_id == '' || patient_id == null || patient_id == undefined){
            alert("Please select patient...");
            $("#patient_id").siblings().children().children().css('background-color', '#FFCECE');  
        }
        else{

            $.ajax({
                url: "/getPatient_detls",
                type: "GET",
                ...
                success: function (data) {
                    ...
                    if(data.status == 'SUCCESS'){
                        $.each(data.message, function(index, v){
                            var v_name = v.patient_name;
                            var v_age = v.age;
                            var v_gender = v.gender;
                            var v_blood_grp = v.blood_grp;
                            var v_num = v.cont_number;
                            var v_address = v.address;

                            $('#patient_nm').val(v_name);
                            $('#patient_age').val(v_age);
                            $('#gender').val(v_gender).change();
                            $('#blood_grp').val(v_blood_grp).change();
                            $('#cont_num').val(v_num);
                            $('#address').val(v_address);

                            $('#patient_nm').prop('disabled', true);
                            $('#patient_age').prop('disabled', true);
                            $('#gender').prop('disabled', true);
                            $('#blood_grp').prop('disabled', true);
                            $('#cont_num').prop('disabled', true);
    ...
    $(document).on('click', '#add_test', function(){
        ...
        test_by = $('#test_by option:selected').val();
        ...
        else if(test_by == '' || test_by == null || test_by == undefined)
        {
            alert("Please select test given doctor name...");
            $("#test_by").siblings().children().children().css('background-color', '#FFCECE');  
        }
        ...
        {
            alert("Please select test given doctor name...");
            $("#blood_grp").siblings().children().children().css('background-color', '#FFCECE');  
        }
